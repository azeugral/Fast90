...
function getRedisClient() {
  ...
  if (!url || !token) {
    const err = new Error(
      'Redis não configurado: conecte um banco Upstash Redis ao projeto na Vercel (aba Storage → Create Database → Upstash → Redis) e faça um novo deploy depois.'
    );
...
export async function getStoredTokens() {
  ...
  try {
    ...
    // Redis ainda não conectado: usa o fallback se existir, senão propaga
    // o erro (com uma mensagem clara, não um crash sem explicação).
...
export async function saveTokens(tokens) {
  ...
  await client.set(TOKEN_KEY, tokens);
}
